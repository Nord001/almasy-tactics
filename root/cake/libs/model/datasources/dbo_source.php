 ...
 */
    function showLog($sorted = false) {
        ...
        if (PHP_SAPI != 'cli') {
            print ("<div class=\"cake-sql-div\"><table class=\"cake-sql-log\" id=\"cakeSqlLog_" . preg_replace('/[^A-Za-z0-9_]/', '_', uniqid(time(), true)) . "\" summary=\"Cake SQL Log\" cellspacing=\"0\" border = \"0\">\n<caption>({$this->configKeyName}) {$this->_queriesCnt} {$text} took {$this->_queriesTime} ms</caption>\n");
            print ("<thead>\n<tr><th>Nr</th><th>Query</th><th>Error</th><th>Affected</th><th>Num. rows</th><th>Took (ms)</th><th>File</th></tr>\n</thead>\n<tbody></div>\n");

            foreach ($log as $k => $i) {
                print ("<tr><td>" . ($k + 1) . "</td><td>" . h($i['query']) . "</td><td>{$i['error']}</td><td style = \"text-align: right\">{$i['affected']}</td><td style = \"text-align: right\">{$i['numRows']}</td><td style = \"text-align: right\">{$i['took']}</td><td style = \"text-align: right\">{$i['file']}</td></tr>\n");
            }
            print ("</tbody></table>\n");
        } else {
            foreach ($log as $k => $i) {
 ...
 */
    function logQuery($sql) {
        ...
        for ($i = 0; $i < count($bt); $i++) {
            ...
            if (strpos($frame['file'], 'cake') === false && strpos($frame['file'], 'app_model') === false) {
                $file = $frame['file'];
                $line = isset($frame['line']) ? $frame['line'] : 'no line';
                $function = isset($bt[$i + 1]['function']) ? $bt[$i + 1]['function'] : 'no function';
                break;
            }
        }
 ...
 */
    function close() {
        if (Configure::read() > 1 && Configure::read('ajaxMode') != 1) {
            $this->showLog();
        }
        $this->disconnect();
    }
