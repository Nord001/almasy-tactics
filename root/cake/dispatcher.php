 ...
 */
App::import('Core', array('Router', 'Controller', 'Cache'));
/**
 ...
 */
    function _invoke(&$controller, $params) {
        ...
        // Use gzip encoding whenever possible
        $canCompress = GZIP_COMPRESS && extension_loaded("zlib") && (strpos(env('HTTP_ACCEPT_ENCODING'), 'gzip') !== false);
        if ($canCompress) {
            ob_start();
 ...
 */
    ...
    function cached($url) {
        ...
        if (strpos($url, 'css/') !== false || strpos($url, 'js/') !== false || strpos($url, 'img/') !== false || strpos($url, 'files/') !== false) {
            $isAsset = false;
            $assets = array('js' => 'text/javascript', 'css' => 'text/css', 'gif' => 'image/gif', 'jpg' => 'image/jpeg', 'png' => 'image/png', 'ttf' => 'application/x-font-TrueType', 'svg' => 'image/svg+xml', 'swf' => 'application/x-shockwave-flash', 'txt' => 'text/plain');
            ...
            // If first part of url is fingerprint, strip it out
            if (strpos($url, WEB_FINGERPRINT) !== false) {
                $url = strstr($url, '/');
            ...
            if ($isAsset === true) {
                ...
                if ($ifModifiedSince && Configure::read('debug') == 0) {
                ...
                header("Last-Modified: " . gmdate("D, j M Y G:i:s ", $fileModified) . 'GMT');
                header('Content-Type: ' . $assets[$type]);
                if (Configure::read('debug') == 0)
                    header("Expires: " . gmdate("D, j M Y H:i:s", time() + STATIC_CACHE_TIME) . " GMT"); // One month
                header("Cache-Control: private, max-age=" . STATIC_CACHE_TIME);
                if ($type === 'css' || $type === 'js') {

                    $cacheJavascript = ($type === 'js' && Configure::read('debug') == 0);

                    $file = false;
                    if ($cacheJavascript) {
                        $cacheKey = GenerateCacheKey(JS_CACHE, $url);
                        $file = CacheRead($cacheKey);
                    }

                    if ($file === false) {
                        // Import html so js can make links
