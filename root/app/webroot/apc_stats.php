...
defaults('USE_AUTHENTICATION',1);           // Use (internal) authentication - best choice if
                                            ...
                                            //  this work!
defaults('ADMIN_USERNAME','almasy');            // Admin Username
defaults('ADMIN_PASSWORD','nullnull');      // Admin Password - CHANGE THIS TO ENABLE!!!
...
if (isset($MYREQUEST['IMG']))
{
    ...
    case 2:
        $s=$cache_user['num_hits']+$cache_user['num_misses'];
        $a=$cache_user['num_hits'];

        fill_box($image, 30,$size,50,-$a*($size-21)/$s,$col_black,$col_green,sprintf("%.1f%%",$cache_user['num_hits']*100/$s));
        fill_box($image,130,$size,50,-max(4,($s-$a)*($size-21)/$s),$col_black,$col_red,sprintf("%.1f%%",$cache_user['num_misses']*100/$s));
        break;
...
EOB;
    ...
    echo
        ...
        '<td class=td-1><span class="green box">&nbsp;</span>Hits: ',$cache_user['num_hits'].sprintf(" (%.1f%%)",$cache_user['num_hits']*100/($cache_user['num_hits']+$cache_user['num_misses'])),"</td>\n",
        '</tr>',
        '<tr>',
        '<td class=td-0><span class="red box">&nbsp;</span>Used: ',bsize($mem_used ).sprintf(" (%.1f%%)",$mem_used *100/$mem_size),"</td>\n",
        '<td class=td-1><span class="red box">&nbsp;</span>Misses: ',$cache_user['num_misses'].sprintf(" (%.1f%%)",$cache_user['num_misses']*100/($cache_user['num_hits']+$cache_user['num_misses'])),"</td>\n";
    echo <<< EOB
        </tr>
        </tbody></table>
