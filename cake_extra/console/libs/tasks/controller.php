 ...
 */
    function __interactive($controllerName = false) {
        ...
        $question[] = "Would you like to build your controller interactively?";
        if (file_exists($this->path . $controllerFile .'_controller.php')) {
            $question[] = sprintf("Warning: Choosing no will overwrite the %sController.", $controllerName);
        }
        ...
        if (strtolower($doItInteractive) == 'y' || strtolower($doItInteractive) == 'yes') {
            ...
            $wannaUseScaffold = $this->in("Would you like to use scaffolding?", array('y','n'), 'n');
 ...
 */
    function bakeActions($controllerName, $admin = null, $wannaUseSession = true) {
        ...
        $actions .= "\t\tif (!\$id) {\n";
        if ($wannaUseSession) {
            $actions .= "\t\t\t\$this->Session->setFlash('Invalid " . low($singularHumanName) . ".');\n";
            $actions .= "\t\t\t\$this->redirect(array('action' => 'index'));\n";
        } else {
            $actions .= "\t\t\t\$this->flash('Invalid " . low($singularHumanName) . "', array('action' => 'index'));\n";
        }
        $actions .= "\t\t}\n";
        $actions .= "\t\t\${$singularName} = \$this->{$currentModelName}->find('first', array(\n";
        $actions .= "\t\t\t'conditions' => array(\n";
        $actions .= "\t\t\t\t'{$currentModelName}.id' => \$id,\n";
        $actions .= "\t\t\t),\n";
        $actions .= "\t\t));\n";
        ...
        if ($wannaUseSession) {
            $actions .= "\t\t\t\t\$this->Session->setFlash('" . ucfirst(low($singularHumanName)) . " saved.');\n";
            $actions .= "\t\t\t\t\$this->redirect(array('action' => 'view', \$this->{$currentModelName}->id));\n";
        } else {
            $actions .= "\t\t\t\t\$this->flash('{ucfirst(low($currentModelName))} saved.', array('action' => 'index'));\n";
        }
        $actions .= "\t\t\t} else {\n";
        if ($wannaUseSession) {
            $actions .= "\t\t\t\t\$this->Session->setFlash('Could not save " . low($singularHumanName) . ".');\n";
        }
        $actions .= "\t\t\t}\n";
        $actions .= "\t\t}\n";
        ...
        if ($wannaUseSession) {
            ...
            $actions .= "\t\t\t\t\$this->flash('The " . low($singularHumanName) . " has been saved.', array('action' => 'index'));\n";
        }
        $actions .= "\t\t\t} else {\n";
        if ($wannaUseSession) {
            $actions .= "\t\t\t\t\$this->Session->setFlash('Could not save " . low($singularHumanName) . ".');\n";
        ...
        $actions .= "\t\tif (!\$id) {\n";
        if ($wannaUseSession) {
            $actions .= "\t\t\t\$this->Session->setFlash('Invalid ID for " . low($singularHumanName) . ".');\n";
            $actions .= "\t\t\t\$this->redirect(array('action' => 'index'));\n";
        } else {
        ...
        if ($wannaUseSession) {
            $actions .= "\t\t\t\$this->Session->setFlash('" . ucfirst(low($singularHumanName)) . " deleted.');\n";
            $actions .= "\t\t\t\$this->redirect(array('action' => 'index'));\n";
        } else {
            $actions .= "\t\t\t\$this->flash('" . ucfirst(low($singularHumanName)) . " deleted', array('action' => 'index'));\n";
        }
        $actions .= "\t\t}\n";
        $actions .= "\t}\n";
        $actions .= "\n";
